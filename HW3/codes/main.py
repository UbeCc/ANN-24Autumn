...
parser.add_argument("--runid", type=str, default="run",
    help="Run ID for Weights and Biases. Default: run")
...
def main():
    ...
    if args.test is None:
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            while ed < len(data["train"]):
                ...
                if (batch_num) % 10 == 0:
                    ...
                    if USE_WANDB:
                        wandb.log({"train_loss": np.mean(losses[-10:]), "epoch": epoch, "batch": batch_num})
            ...
            if val_ppl < best_val_ppl:
                ...
                if USE_WANDB:
                    wandb.log({"best_val_ppl": best_val_ppl, "best_epoch": best_epoch})
            else:
        ...
        if USE_WANDB:
            wandb.log({"test_loss": test_loss, "test_ppl": test_ppl})
        ...
        with open(f"output_{args.runid}_{args.decode_strategy}.txt", "w") as fout:
