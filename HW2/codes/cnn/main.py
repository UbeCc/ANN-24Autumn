...
parser.add_argument('--learning_rate', type=float, default=1e-2,
	help='Learning rate during optimization. Default: 5e-4')
parser.add_argument('--drop_rate', type=float, default=0.2,
	help='Drop rate of the Dropout Layer. Default: 0.2')
...
def valid_epoch(model, X, y, test): # Valid Process
	model.eval()
	loss, acc = 0.0, 0.0
	st, ed, times = 0, args.batch_size, 0
	while st < len(X) and ed <= len(X):
...
if __name__ == '__main__':
	...
	if args.is_train:
		...
		for epoch in range(1, args.num_epochs+1):
			...
			val_acc, val_loss = valid_epoch(mlp_model, X_val, y_val, test=False)

			if val_acc >= best_val_acc:
				best_val_acc = val_acc
				best_epoch = epoch
				test_acc, test_loss = valid_epoch(mlp_model, X_test, y_test, test=True)
